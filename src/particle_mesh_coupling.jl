...
function add_current_update_v_pp!(
    ...
    if index_old == index_new
        ...
        vi = update_jv_pp!(j_dofs, p, r_old, r_new, index_old, marker_charge, qoverm, vi)

    elseif index_old < index_new

        vi = update_jv_pp!(j_dofs, p, r_old, 1.0, index_old, marker_charge, qoverm, vi)

        vi = update_jv_pp!(j_dofs, p, 0.0, r_new, index_new, marker_charge, qoverm, vi)

        for ind = index_old+1:index_new-1
            vi = update_jv_pp!(j_dofs, p, 0.0, 1.0, ind, marker_charge, qoverm, vi)
        end

    else

        vi = update_jv_pp!(j_dofs, p, 1.0, r_new, index_new, marker_charge, qoverm, vi)

        vi = update_jv_pp!(j_dofs, p, r_old, 0.0, index_old, marker_charge, qoverm, vi)

        for ind = index_new+1:index_old-1
            vi = update_jv_pp!(j_dofs, p, 1.0, 0.0, ind, marker_charge, qoverm, vi)
        end
...
function update_jv!(
    ...
    p.spline_val .= uniform_bsplines_eval_basis(p.spline_degree, c1 * p.quad_x[1] + c2)
    ...
    for j = 2:p.n_quad_points
        ...
        p.spline_val_more .=
            uniform_bsplines_eval_basis(p.spline_degree, c1 * p.quad_x[j] + c2)
